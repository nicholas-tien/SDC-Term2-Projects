...
//    std::cout << "Hello, World!" << std::endl;
    ...
    /**
     * Sigma - just an estimate, usually comes from uncertainty of sensor,
     * but if you used fused data from multiple sensors, it's difficult fo find
     * these uncertainties directly
     */
    double sigma_pos[3] = {0.3, 0.3, 0.01}; // GPS measurement uncertainty [x [m], y [m], theta [theta]]
    ...
    if (!read_control_data("../data/control_data.txt",position_meas)) {
        ...
        return  -1;  // return anything except 0 means there is an error
    }
    ...
    if (!read_gt_data("../data/gt_data.txt",gt)) {
        cout << "Error: Could not open ground truth data fiel" << endl;
        return -1;
    }
    ...
    for (int i = 0; i < num_time_steps; ++i) {
        ...
        // Read in landmark observation for current time step.
        ostringstream file;
...
//        cout << file.str() << endl;
        ...
        // Initialize particle filter if this the first time step.
        if (!pf.initialized()) {
            n_x = N_x_init(gen);
            n_y = N_y_init(gen);
    ...
    // Print success if accuracy and runtime are sufficient (and this isn't the starter code)
